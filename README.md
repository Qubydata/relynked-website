...
## Files
    ...
    industries/             "Who Relynked is for" pages, GENERATED, do not edit by hand
    scripts/build-industries.mjs      builds industries/, sitemap.xml and robots.txt
    scripts/industries-fallback.json  business types to use if the live API is down or incomplete
    sitemap.xml, robots.txt GENERATED by the same script

## Who it is for pages (industries)

The pages under `industries/` are built from the app's own list of business types, so the
site always lists what the app supports. After a business type is added, renamed or removed
in the app, run this from this folder, check the result, then deploy as usual:

    node scripts/build-industries.mjs

It needs Node 18 or newer and nothing else. It reads
https://relynked-api-production.up.railway.app/business-types (no login). If that is
unreachable, or does not yet return categories and aliases, it fills the gaps from
`scripts/industries-fallback.json`. It prints which source it used.

What it writes: `industries/index.html`, one `industries/<slug>.html` per business type
(the "Something else" type gets no page), `sitemap.xml` (every root page except
`dashboard.html` and anything marked noindex, plus the industries pages) and `robots.txt`.
Pages for types that no longer exist are deleted. The head, nav and footer are copied from
`pricing.html` on every run, so after changing the footer, re-run the script.

The copy for each page comes from the kind of business: menu types get food copy, service
types (barber, salon) get bookings copy, stock types get shop copy. The `HINTS` table at
the top of the script fixes the odd plural or URL; a new type needs no entry. A page's URL
comes from its label (`Barber shop` becomes `/industries/barber-shop`), so renaming a
type in the app changes its URL unless a slug hint pins it.

Also run the script after adding a new page to the site, so the sitemap lists it.
